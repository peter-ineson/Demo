...
public class SolarBodyLinkEndpoint {
    ...
    public Response findLinksBySolarBodyId(
            @HeaderParam("accept") String accept,
            @PathParam("solarBodyId") Long solarBodyId) {
        log.debug( "Looking up links for solar body id " + solarBodyId);

        WebTarget target = CLIENT.target( config.getStringManadtory( Config.SERVICE_REST_URL))
                .path(SolarBodyEndpoint.PATH).path( solarBodyId.toString()).path("links");
        ...
        if( response.getStatus() == Status.OK.getStatusCode()) {
            ...
            log.error( "Unknown response findLinksBySolarBodyId lookup url {}, response {}\n{}",
